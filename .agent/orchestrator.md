...
encertia/
  ...
  .agent/
    ...
    ux-agent.md                     -> àmbit (frontend, experiència d'ús),
                                        com avalua usabilitat i coherència visual
  backend/
  ...
  qa-reports/
    ...
    ...
  ux-reports/
    auth.md                     -> informe UX del mòdul auth (APTE/A MILLORAR)
    ...
```
...
## 7. Flux de treball multiagent (orquestrador + picacodis + infra + QA + UX)

Aquest projecte es desenvolupa amb **sis rols d'agent diferenciats**:
...
### Rol 6 — Agent UX
- Actua **en paral·lel o just després de l'agent QA**, sobre el mateix
  mòdul ja funcionant en local. Mentre QA valida que el mòdul *funcioni*
  correctament (backend + frontend + homogeneïtat de codi), l'Agent UX
  valida que **es visqui bé**: fluïdesa, claredat, facilitat d'ús.
- Només revisa `frontend/` (i com el frontend consumeix el backend, mai
  el codi backend en si mateix). No jutja qualitat de codi ni arquitectura
  — això és terreny del QA.
- **Prova l'aplicació de veritat en local** (igual que el QA), simulant
  l'ús real d'un professor i d'un alumne, no només mirant el codi Vue.
- Eixos de revisió:
  1. **Usabilitat**: els fluxos es completen sense fricció innecessària?
     Un usuari nou (professor no tècnic, o alumne al mòbil) entendria què
     fer sense explicació prèvia?
  2. **Consistència visual**: es fa servir PrimeVue de manera coherent
     (mateixos components per als mateixos casos d'ús a tot el projecte,
     no un `Dialog` en un mòdul i un modal fet a mà en un altre)?
  3. **Responsive/mòbil**: especialment crític als fluxos on l'alumne
     interactua des del mòbil (partida en directe, guió de classe) —
     es veu i es fa servir bé en una pantalla petita?
  4. **Feedback a l'usuari**: l'aplicació informa clarament d'èxits,
     errors i estats de càrrega (no deixa l'usuari "penjat" sense saber
     si una acció ha funcionat)?
  5. **Accessibilitat bàsica**: contrast de color suficient, mida de
     text llegible, elements clicables prou grans al mòbil.
- **No corregeix codi ell mateix.** Documenta incidències i les escala a
  l'Orquestrador, igual que el QA.
- **Àmbit d'escriptura**: només `ux-reports/<modul>.md`. Prohibit escriure
  a `frontend/`, `backend/`, `contracts/` o qualsevol `.agent/*.md`.
- Cada informe conclou amb un veredicte: **APTE** o **A MILLORAR** (amb
  incidències classificades per impacte: crític / notable / detall).
  A diferència del QA, una incidència d'UX rarament bloqueja el merge per
  ella sola — el veredicte "A MILLORAR" és informatiu per priorizar
  iteracions, no necessàriament un bloqueig com el "NO APTE" del QA
  (l'humà decideix si bloqueja o no, cas per cas).
